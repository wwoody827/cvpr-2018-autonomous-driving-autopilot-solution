...
def printProgressBar (iteration, total, prefix = '', suffix = '', decimals = 1, length = 200, fill = '█'):
    """
...
class SENet_backbone(nn.Module):
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
                 inplanes=128, input_3x3=True, downsample_kernel_size=3,
                 downsample_padding=1, stage5=False):
        
        super(SENet_backbone, self).__init__()
...
def se_resnext101_32x4d(stage5=False):
    model = SENet_backbone(SEResNeXtBottleneck, [3, 4, 23, 3], groups=32, reduction=16,
...
def refine_detections(rois, probs, deltas, window, config):
    ...
    if len(list(torch.nonzero(keep_bool).size())) != 0:
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, image_index):
        ...
        image, image_metas, gt_class_ids, gt_boxes, gt_masks = \
            load_image_gt(self.dataset, self.config, image_id, augment=self.augment, augmentation = self.augmentation,
...
class MaskRCNN(nn.Module):
    ...
    def load_weights(self, filepath, strict = False):
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs, layers, augmentation = None):
        ...
        # Data generators
        train_set = Dataset(train_dataset, self.config, augment=False, augmentation = augmentation)
        train_generator = torch.utils.data.DataLoader(train_set, batch_size=1, shuffle=True, num_workers=8)
        val_set = Dataset(val_dataset, self.config, augment=False)
        val_generator = torch.utils.data.DataLoader(val_set, batch_size=1, shuffle=True, num_workers=8)
        ...
        if self.config.OPTIMIZER == 'SGD':
            ...
            optimizer = optim.Adam([
                {'params': trainables_wo_bn, 'weight_decay': self.config.WEIGHT_DECAY},
                {'params': trainables_only_bn}
            ], lr=learning_rate, eps=self.config.EPSILON)
    ...
    def train_epoch(self, datagenerator, optimizer, steps):
        ...
        step = 0
        print_iter = min(int(np.floor(steps/10)), self.config.PRINT_ITER)
        ...
        for inputs in datagenerator:
            ...
            loss = 1/self.config.BATCH_SIZE * (rpn_class_loss + rpn_bbox_loss + mrcnn_class_loss + mrcnn_bbox_loss + mrcnn_mask_loss)
            ...
            # Statistics
            loss_sum += loss.data.cpu()[0]/steps * self.config.BATCH_SIZE
            ...
            if (step + 1) % print_iter == 0:
                time_used = time.time() - start_time
                img_persecond = print_iter/time_used
                start_time = time.time()
                eta = (steps - step)/img_persecond/60
                printProgressBar(step + 1, steps, prefix="\t{}/{}".format(step + 1, steps),
                                 suffix="Complete - loss: {:.5f} - rpn_class_loss: {:.5f} - rpn_bbox_loss: {:.5f} - mrcnn_class_loss: {:.5f} - mrcnn_bbox_loss: {:.5f} - mrcnn_mask_loss: {:.5f} - image_persecond: {:1.3f} - eta(min): {:1.3f}".format(
                                     loss_sum*steps/(step + 1), loss_rpn_class_sum*steps/(step + 1), loss_rpn_bbox_sum*steps/(step + 1),
                                     loss_mrcnn_class_sum*steps/(step + 1), loss_mrcnn_bbox_sum*steps/(step + 1),
                                     loss_mrcnn_mask_sum*steps/(step + 1), img_persecond, eta), length=1)            
    ...
    def valid_epoch(self, datagenerator, steps):
        ...
        for inputs in datagenerator:
            ...
            if step==steps-1:
                printProgressBar(step + 1, steps, prefix="\t{}/{}".format(step + 1, steps),
                             suffix="Complete - loss: {:.5f} - rpn_class_loss: {:.5f} - rpn_bbox_loss: {:.5f} - mrcnn_class_loss: {:.5f} - mrcnn_bbox_loss: {:.5f} - mrcnn_mask_loss: {:.5f}".format(
                                 loss_sum, loss_rpn_class_sum, loss_rpn_bbox_sum,
                                 loss_mrcnn_class_sum, loss_mrcnn_bbox_sum,
                                 loss_mrcnn_mask_sum), length=10)
                break
            step += 1
    ...
    def unmold_detections(self, detections, mrcnn_mask, image_shape, window):
        ...
        for i in range(N):
            ...
            full_mask = utils.unmold_mask(masks[i], boxes[i], image_shape, threshold = self.config.MASK_THRESHOLD)
...
def mold_image(images, config):
    ...
    colors in RGB order.
    """
    image = images.astype(np.float32)/255.0 - config.MEAN_PIXEL
    for i in range(3):
        image[:, :, i] = image[:, :, i] / config.STD[i]
    return image
