...
class AdrivingDatasetPadding(utils.Dataset):
    def mask_to_instance(self, mask):
        instances_all = np.unique(mask)
        instances = []
        for i in range(len(instances_all)):
    ...
    def load_image(self, image_id):
        ...
        img_crop = np.zeros([self.size[0], 3584, 3], dtype = np.float)
        img_crop[:, 100:(100+3384), :] = image[-self.size[0]:, :, :]
        return img_crop
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        # If not a balloon dataset image, delegate to parent class.
        img_path = self.image_info[image_id]['path']
        mask_path_filename = get_label_name(os.path.basename(img_path))
        ...
        # [height, width, instance_count]
        mask_raw = np.array(skimage.io.imread(mask_path))
        mask_crop = np.zeros([self.size[0], 3584], dtype = int)
        mask_crop[:, 100:(100+3384)] = mask_raw[-self.size[0]:, :]
...
class AdrivingDatasetNoResize(utils.Dataset):
    ...
    def load_image(self, image_id):
        ...
        for i in range(300):
            # print(i)
            crop_start = (np.random.randint(300, crop_max[0]), 
            ...
            # print(i, np.sum(mask_crop != 255))
            # if np.sum(mask_crop != 255) >= 100:
            #     break
...
def instance_to_mask(instance_mask, class_ids, score = None, order_by_score = True):
